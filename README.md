...
自動判定して変換します。カルーセル付きTSは読みながら書くので、入力の長さに関わらず
メモリは先読みの長さ（`-window`、既定20秒）で頭打ちになります。カルーセルのないTSは
入力全体のPSI/SIからMH-SIを組むため、読み切ってから書きます。カルーセルのないTSから変換できるのは、HEVC映像、AAC音声、
および対応するPSI/SIです。
